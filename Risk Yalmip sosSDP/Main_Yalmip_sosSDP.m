...
%% Yalmip
% Yalmip solves the sum-of-squares SDP.
% sum-of-squares SDP looks for the coefficients of the polynomial indicator function.
[c_sos]=func_Yalmip(nx,x,g,d,yL);
Risk_sos=sum(yx1x2.*c_sos);

% polynomial indicator function obtianed from the solution of Dual SDP
z=sym('x',[1 nx]); P=sum(prod(z.^vpow,2).*c_sos); 
[x1,x2] = meshgrid(-0.9:0.01:0.9,-0.9:0.01:0.9);P=eval(P);
close all;surfc(x1,x2,P,'FaceColor','green','EdgeColor','none','FaceAlpha',0.7);
...
%% Compare
clc; 
display('   Yalmip_sos,     monte carlo')
display([Risk_sos,            Risk_monte])
